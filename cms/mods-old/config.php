...
function Main()
{
	...
	$query = "SELECT * FROM " . $db_table . " WHERE tipo='dev' ORDER BY id";
	$admin->breadcrumbs();
	$admin->pageTitle();
			...
			<table class="list-table">
				<thead>
					<tr>
						...
						<th class="hide-for-small">Conteúdo</th>
					</tr>
				</thead>
				<tbody>';
...
function Form($id)
{
	...
	if($in == 'editar')
	{
		$arr = mysql_fetch_array(mysql_query("SELECT * FROM " . $db_table . " WHERE tipo='dev' && id='" . $id . "' LIMIT 1"));
	}
	echo '
	<div class="row">
	  <form method="post" action="index.php?on=' . $on . '">
	    ...
	    <fieldset>
		  ...
		  <div class="row">
		    <div class="large-12 columns">
		      <label>Conteúdo';
		      if($arr['campo'] == 'textarea')
		      {
		      	echo '
		        <textarea name="valor" placeholder="Conteúdo" class="height100">' . $arr['valor'] . '</textarea>';
		      }else
		      {
		      	echo '
