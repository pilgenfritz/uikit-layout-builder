...
/*
	...
	Utilizado no módulo Como Adquirir
		...
		<div class="columns large-12">';
			if(mysql_num_rows(mysql_query($query)) > 0)
			{
				...
				<table id="tabela_menu" class="list-table">
					...
					{
						echo '
						<tr id="' . $arr['id'] . '" class="ui-state-default'; if($arr['active'] != 'Y') echo ' '; echo '">
							<td>
								<a href="index.php?on=' . $on . '&in=editar&id=' . $arr['id'] . '">
									' . strip_tags($arr['titulo']) . '
								</a>
							</td>
							<td class="text-right">';
								echo date('d/m/Y',strtotime($arr['data']));
	  ...
	  <form method="post" action="index.php?on=' . $on . '" enctype="multipart/form-data">
	    ...
	    <fieldset>
		  ...
		  <div class="row">
		    <div class="large-12 columns">
		      <label><strong>Indústria/Produto</strong></label>
		      <p>'. $arr['titulo'] .'</p>
		    </div>
		  </div>
		  <div class="row">
		  ...
		  <div class="row">
			...
			<hr/>
			<h5>Respostas ao Questionário</h5>
			<br/>
		  <div class="row">
		    <div class="large-12 columns">
		      '. $arr['questionario'] .'
		  ...
		  $planoSelecionado = mysql_fetch_array(mysql_query("SELECT * FROM planos WHERE id='" . $arr['plano'] . "' LIMIT 1"));
		  echo '
			<hr/>
		  <div class="row">
		    <div class="large-12 columns">
		      <strong>Plano Selecionado:</strong> '. $planoSelecionado['titulo'] .'
		    </div>
		  </div>';
		...
		<div class="row">
			<div class="large-12 columns margintop20 text-right">';
		    	...
		    	<button type="submit" class="hide">' . $admin->formButton() . '</button>
			</div>
		</div>
	  </form>
	</div>';
				...
				<div class="tabs-content show-code">
				  <div class="content active" id="panel2-1">
					<div class="mod-options">
						...
						<label for="imagem_cat">
							<input type="checkbox" name="imagem_cat" id="imagem_cat" value="Y" '; if($admin->ModuloOptionsCheck($on,'imagem_cat')) echo ' checked'; echo '/> Imagem na Categoria
						</label>
						<label for="imagem">
						...
						<label for="descricao_cat">
							<input type="checkbox" name="descricao_cat" id="descricao_cat" value="Y" '; if($admin->ModuloOptionsCheck($on,'descricao_cat')) echo ' checked'; echo '/> Descrição (Categoria)
						</label>
						<label for="galeria">
