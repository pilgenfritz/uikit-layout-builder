		...
		<div class="columns large-12">
			<table class="list-table">
				<thead>
					<tr>
						<th>Chave</th>
						<th class="hide-for-small">Tabela</th>
						<th class="hide-for-small">Valor</th>
					</tr>
				</thead>
				<tbody>';
				...
				while ($arr = mysql_fetch_array($rr))
				{
					echo '
					<tr>
						<td class="has-tip" data-tooltip title="' . $arr['descricao'] . '">
							<a href="index.php?on=' . $on . '&in=editar&id=' . $arr['id'] . '">' . $arr['chave'] . '</a>
						</td>
						<td class="hide-for-small">
							<a href="index.php?on=' . $on . '&in=editar&id=' . $arr['id'] . '">' . $arr['tabela'] . '</a>
						</td>
						<td class="hide-for-small">
	  ...
	  <form method="post" action="index.php?on=' . $on . '">
	    ...
	    <fieldset>
		  ...
		  <div class="row">
		    <div class="large-12 columns">
		      <label>Tabela do db
		        <input type="text" name="tabela" placeholder="Tabela do db" value="' . $arr['tabela'] . '" required />
		      </label>
		    </div>
		  </div>
		  <div class="row">
		    <div class="large-12 columns">
		      <label>Valor
		        <input type="text" name="valor" placeholder="Conteúdo" value="' . $arr['valor'] . '" required />
		      </label>
		    </div>
		  </div>
		  <div class="row">
		...
		<div class="row ">
			<div class="large-12 columns margintop20 text-right">
				<a class="hide-for-small" href="index.php?on=' . $on . '&in=apagar&id=' . $arr['id'] . '" class="button alert marginright10 left">Apagar</a>
